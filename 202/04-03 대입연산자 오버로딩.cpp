...
public:
    ...
    {
        cout << "HighScoolStudent 일반생성자 호출" << endl;
    }

    //소멸자
    ~HighSchoolStudent()
    {
        cout << "HighScoolStudent 소멸자 호출" << endl;
    }
private:
    string sHakgwa;
};
...
// 객체생성과 동시에 멤버변수 초기화 //멤버변수(매개변수) 형식
// const/참조형 멤버변수를 사용할 수 있다.
    : nHakbun(Hakbun)
{
    cout << "Student 일반생성자가 호출" << endl;
    int len = strlen(Name) + 1; // 공간개수 측정  // 동적할당할 공간의 크기를 구하고 
    sName = new char[len];      // 공간생성 // 그 크기만큼 배열로 동적할당
